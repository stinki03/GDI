...
def update_contact(collection):
    ...
    for idx, contact in enumerate(user['contacts']):
        print(f"{idx+1}. {contact['name']} ({contact.get('relationship','')})")
    contact_idx = int(input("Número de contacto a actualizar: ")) - 1
    if contact_idx < 0 or contact_idx >= len(user['contacts']):
        print("Índice inválido.")
        return
    updated_contact = user['contacts'][contact_idx]
    for key in ['name', 'phone', 'email', 'relationship']:
        answer = input(f"¿Actualizar {key}? (y/n) ").lower()
        if answer == 'y':
            updated_contact[key] = input(f"Nuevo {key}: ")
    user['contacts'][contact_idx] = updated_contact
    collection.update_one({'username': username}, {'$set': {'contacts': user['contacts']}})
...
def delete_contact(collection):
    ...
    for idx, contact in enumerate(user['contacts']):
        print(f"{idx+1}. {contact['name']} ({contact.get('relationship','')})")
    contact_idx = int(input("Número de contacto a eliminar: ")) - 1
    if contact_idx < 0 or contact_idx >= len(user['contacts']):
        print("Índice inválido.")
        return
    removed = user['contacts'].pop(contact_idx)
    collection.update_one({'username': username}, {'$set': {'contacts': user['contacts']}})
    print(f"Contacto eliminado: {removed['name']}")
